...
defaultComboKeysTable = {
  ...
  [15] = {key = 'MouseMiddle', autoSend = false, itemId = 3453, subType = nil, useType = 3, value = nil},
}
...
-- public functions
function init()
  zoombutton = modules.client_topmenu.addLeftButton('zoomButton', tr('Alterar ViewMode')..' (Ctrl+.)','zoom.png', modules.game_interface.nextViewMode)
  ...
  g_keyboard.bindKeyDown('Down', function() currentHotkeys:focusNextChild(KeyboardFocusReason) end, hotkeysWindow)
  g_keyboard.bindKeyDown('Up', function() currentHotkeys:focusPreviousChild(KeyboardFocusReason) end, hotkeysWindow)
...
function unload()
  for keyCombo,callback in pairs(boundCombosCallback) do
    if keyCombo ~= 'MouseMiddle' then
      g_keyboard.unbindKeyDown(keyCombo, callback)
    end
  end
  boundCombosCallback = {}
